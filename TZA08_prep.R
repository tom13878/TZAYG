...
geo.vars <- read.dta("./Data/Tanzania/2008_09/Stata/TZNPS1_consdta/HH.Geovariables_Y1.dta", convert.factors = TRUE)
geo.vars <- left_join(geo.vars, select(HQSECA1, hhid, region), by = "hhid") %>%
...
geo.vars$zone[geo.vars$region %in% c("KASKAZINI UNGUJA", "KUSINI UNGUJA", "MJINI/MAGHARIBI UNGUJA", "KASKAZINI PEMBA", "KUSINI PEMBA")] <- "Zanzibar"
...
hh.char <- select(HQSECBU, hhid, member = sbmemno, status = sbq5, sex = sbq2, age = sbq4) %>%
  ddply(.(hhid), transform, hh.size = length(member)) %>% filter(status == "HEAD") %>% select(-(member:status))
hh.cap <- ddply(AQSEC11, .(hhid), summarize, cap.own = plus(s11q1 * s11q2), cap.rent = plus(s11q7 * s11q9))
hh.cap$cap.rent[is.na(hh.cap$cap.rent)] <- 0 
hh.plots <- ddply(AQSEC3A, .(hhid), summarize, plots = sum(!is.na(plotnum)), plot.missing = factor(missing.plot(s3aq5code)))
...
AQSEC3A <- read.dta("./Data/Tanzania/2008_09/Stata/TZNPS1AGDTA_E/SEC_3A.dta", convert.factors = FALSE)
region_and_zone <- read.csv("./Analysis/Cleaned_data/region_and_zone.csv", colClasses = c("character", "factor", "factor"))
plot.geo <- read.dta("./Data/Tanzania/2008_09/Stata/TZNPS1_consdta/Plot.Geovariables_Y1_revised2.dta", convert.factors = TRUE)

plot.vars <- left_join(AQSEC2A, AQSEC3A) %>% left_join(region_and_zone) %>%
  left_join(plot.geo, by = c("hhid", 'plotnum'))
...
plot.vars <- transmute(plot.vars, hhid, plotnum, region, zone, hir.lab.days,
                     ...
                     pest.type = factor(s3aq50, labels = c("Pesticide", "Herbicide", "Fungicide", "Other")),
...
fert.vars <- left_join(AQSEC3A, region_and_zone) %>%
  transmute(hhid, plotnum, region, zone, 
            ...
            fert.type = factor(s3aq44, labels = c("DAP", "UREA", "TSP", "CAN", "SA", "generic NPK (TZA)", "MRP"))) %>%
  arrange(fert.type)

fert_comp <- read.xls("Data/Other/Fert_comp.xlsx", sheet = 2) %>% rename(fert.type = Fert_type2)
fert_comp$P_share <- as.numeric(fert_comp$P_share)
fert_comp$K_share <- as.numeric(fert_comp$K_share)

fert.vars <- left_join(fert.vars, select(fert_comp, fert.type, N_share, P_share, K_share)) # split this up or R crashes
...
prices <- ddply(prices, .(region, itemname), function(elt) winsor(elt, 0.975, "pricePkg"))

# Prices <- select(prices_winsor, region, itemname, PricesEA, year)
prices.region <- ddply(prices, .(region, itemname), summarize, region.price = mean(pricePkg, na.rm = TRUE), num.obs = sum(is.na(pricePkg)))
prices.national <- ddply(prices, .(itemname), summarize, national.price = mean(pricePkg, na.rm = TRUE))
...
count <- ddply(count, .(hhid, plotnum), summarize, crop.count = length(unique(value[!is.na(value)])))
output.maize <- left_join(output.maize, count)
 
output.maize <- left_join(output.maize, region_and_zone)
output.maize <- left_join(output.maize, select(crop.codes, CropName, itemid, itemname, CashCrop), by = c("zaocode" = "CropName"))
