...
by_hhid <- group_by(RS, y3_hhid)
hhid_summarise <- summarise(by_hhid,
...
comp <- left_join(comp, hhid_summarise)
...
comp$plotnum <- unclass(comp$plotnum)
...
# -------------------------
# run the amelia package
# --------------------------
library(Amelia)
a.out <- amelia(comp, m = 5, cs = "y3_hhid")
summary(a.out$imputations[[5]]$area.gps) # have a look at the result of the
                                         # 5th imputation

# --------------------------
# recombine the datasets by taking averages
# --------------------------

# recombining, following Palacios-Lopez and Yacoubou Djima 2014, just take the
# average of the imputations
x1 <- a.out$imputations[[1]]$area.gps
x2 <- a.out$imputations[[2]]$area.gps
x3 <- a.out$imputations[[3]]$area.gps
x4 <- a.out$imputations[[4]]$area.gps
x5 <- a.out$imputations[[5]]$area.gps

area.gps.imp <- (x1 + x2 + x3 + x4 + x5)/5


# attach to comp dataset
comp$area.gps.imp <- area.gps.imp
comp$area <- ifelse(is.na(comp$area.gps), comp$area.gps.imp, comp$area.gps)
comp <- mutate(comp, area = area*0.40468564224) # original data is in acres
# --------------------------
# explore data
# --------------------------

# --------------------------
# Use the imputed variables for are.gps
# where there is no original gps measurment
# ---------------------------
