...
head <- select(HQSECBU, hhid, sbmemno, sbq5, sbq2, sbq4) %>%
  ...
  select(hhid, soh = sbq2, aoh = sbq4, hh.size)
...
hh.total <- left_join(hh.total, hhid.region)
...
fert.vars <- mutate(fert.vars, nitrogen.kg = fert.kg * (N_share/100),
                    phosphorous.kg = fert.kg * (P_share/100), potassium.kg = fert.kg * (K_share/100)) %>%
